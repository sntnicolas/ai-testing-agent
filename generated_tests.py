...
def test_endpoint_no_auth():
    response = requests.get(f"{BASE_URL}/api/endpoint?param=other")
    assert response.status_code == 401

def test_endpoint_invalid_api_key():
    headers = {"Authorization": "Bearer invalid-api-key"}
    response = requests.get(f"{BASE_URL}/api/endpoint?param=other", headers=headers)
    assert response.status_code == 403

def test_endpoint_random_auth_key():
    headers = {"Authorization": "Bearer some-valid-looking-key"}
    response = requests.get(f"{BASE_URL}/api/endpoint?param=other", headers=headers)
